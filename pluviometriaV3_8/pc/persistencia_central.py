...
class PersistenciaCentralSQLite:
    ...
    def __init__(
        ...
        # Criar a conexão com o banco SQLite para ser compartilhada
        self.conexao = sqlite3.connect(CAMINHO_BANCO_CENTRAL_PC, check_same_thread=False)
        
        # Agora passa a conexão para o repositório
        self.repositorio = repositorio or RepositorioCentralSQLite(conexao=self.conexao)
        self.repositorio.inicializar_banco()

        self.repositorio_analitico = repositorio_analitico or RepositorioAnaliticoSQLite(conexao=self.conexao)
        self.repositorio_analitico.inicializar_banco()

        self.repositorio_alertas_ciclo = repositorio_alertas_ciclo or RepositorioAlertasCicloSQLite(conexao=self.conexao)
