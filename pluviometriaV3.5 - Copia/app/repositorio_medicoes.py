...
class RepositorioMedicoesSQLite:
    ...
    def remover_todas_medicoes(self) -> int:
        """Remove todas as medições do banco local."""
        with sqlite3.connect(self.caminho_banco) as conexao:
            cursor = conexao.cursor()
    ...
    def remover_medicoes_enviadas_mais_antigas_que(self, data_limite: str) -> int:
        """
        Remove medições com status ENVIADO cujo enviado_em seja anterior à data limite.
        A data deve estar no formato YYYY-MM-DD HH:MM:SS.
        """
        ...
        with sqlite3.connect(self.caminho_banco) as conexao:
            ...
            cursor.execute(
                ...
                AND enviado_em < ?
                """,
                (STATUS_ENVIADO, data_limite.strip()),
            )
            conexao.commit()
            return int(cursor.rowcount)
        
    def contar_medicoes_enviadas_mais_antigas_que(self, data_limite: str) -> int:
        """
        Conta medições com status ENVIADO cujo enviado_em seja anterior à data limite.
        A data deve estar no formato YYYY-MM-DD HH:MM:SS.
        """
        if not data_limite or not data_limite.strip():
            raise ValueError("A data limite é obrigatória.")
